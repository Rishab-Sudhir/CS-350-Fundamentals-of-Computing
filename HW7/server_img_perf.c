...
// Including methods for counting instructions
#include "perflib.h"
...
void * worker_main (void * arg)
{
	...
	int evt_fd = -1; // Event file descriptor
	uint64_t type, config;
	
	// Check if event profiling is enabled
	if (params->event_name != NULL) {
		if (strcmp(params->event_name, "INSTR") == 0) {
			type = PERF_TYPE_HARDWARE;
			config = PERF_COUNT_HW_INSTRUCTIONS;
		} else if (strcmp(params->event_name, "L1MISS") == 0) {
			type = PERF_TYPE_HW_CACHE;
			config = (PERF_COUNT_HW_CACHE_L1D) |
					(PERF_COUNT_HW_CACHE_OP_READ << 8) |
					(PERF_COUNT_HW_CACHE_RESULT_MISS << 16);
		} else if (strcmp(params->event_name, "LLCMISS") == 0) {
			type = PERF_TYPE_HW_CACHE;
		...
		if (evt_fd == -1) {
			ERROR_INFO();
			perror("setup_perf_counter error");
			// Handle the error, possibly exit the thread
            pthread_exit(NULL);
		} else {
	...
	while (!params->worker_done) {
		...
		// Reset the counter before the image operation
		if (evt_fd != -1 && req.request.img_op != IMG_REGISTER) {
			ioctl(evt_fd, PERF_EVENT_IOC_RESET, 0);
		}
		
		// process the image operation
		...
		// Read the counter after the image operation
		uint64_t event_count = 0;
		if (evt_fd != -1 && req.request.img_op != IMG_REGISTER) {
			event_count = read_perf_counter(evt_fd);
		}
		...
		/* In worker_main, after processing the request and before calling dump_queue_status */
		    ...
		    struct worker_params * common_params)
{
	...
	if (cmd == WORKERS_START) {
		...
		for (i = 0; i < worker_count; ++i) {
			...
			worker_params[i]->event_name = common_params->event_name;
		}
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	common_worker_params.event_name = conn_params.event_name; // Setting the event_name parsed in main
...
int main (int argc, char ** argv) {
	...
	char *event_name = NULL; // To pass in the event name as the -h flag
	...
	*/
	...
	while((opt = getopt(argc, argv, "q:w:p:h:")) != -1) {
		...
		case 'h':
			event_name = optarg;
			printf("INFO: Setting event name = %s\n", event_name);
			//validate the event_name
			if (strcmp(event_name, "INSTR") != 0 &&
                strcmp(event_name, "L1MISS") != 0 &&
                strcmp(event_name, "LLCMISS") != 0) {
                ERROR_INFO();
                fprintf(stderr, "Invalid event name.\n" USAGE_STRING, argv[0]);
                return EXIT_FAILURE;
            }
            break;
		default: /* '?' */
	...
	// Assign event_name to conn_params
