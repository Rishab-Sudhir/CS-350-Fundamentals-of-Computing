...
# Load the server log
file_path = './1b_Server_Output.txt'
...
# Normal Fit
norm_params2 = norm.fit(request_lengths)

# Mean and STD
norm_mean2, norm_std2 = norm_params2
norm_pdf2 = norm.pdf(np.linspace(request_lengths.min(), request_lengths.max(), 100), *norm_params2)
print(f"Norm mean: {norm_mean2}, STD: {norm_std2}")
...
plt.plot(np.linspace(request_lengths.min(), request_lengths.max(), 100), norm_pdf2, 'b-', lw=2, label='Uniform Fit')
